...
export function PlayNow({ game_slug, name: _name }: { game_slug: string; name: string }) {
    ...
    return (<div className="group relative inline-flex">
        ...
        <button
            ...
            className="play-now-shine relative inline-flex items-center justify-center gap-2.5 rounded-full border-2 border-white/35 bg-[#2458d3]  px-4 py-2 font-bold text-white shadow-[0px_10px_20px_rgba(15,23,42,0.24)] outline-none transition-all duration-300 hover:scale-[1.03] hover:border-sky-200/75 active:scale-[0.985]"
        >
            <span className="inline-flex h-6 w-6 lg:h-8 lg:w-8 shrink-0 items-center justify-center rounded-full bg-white/95 text-[#1d4db8] shadow-[0_0_12px_rgba(255,255,255,0.28)] transition-transform duration-300 group-hover:translate-x-0.5">
                <PlayIcon className="h-4.5 w-4.5" />
            </span>
            <span className="flex flex-col gap-0.5 text-left leading-none">
                <span className="text-xs font-bold  lg:text-[15px]">Play Now</span>
                <span className="text-[8px] lg:text-[9px] font-bold uppercase tracking-[0.16em] text-blue-100/90">Competitive Match</span>
            </span>
        </button>
