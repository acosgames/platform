...
export function StatsLeaderboardPlayerRow({
  ...
  const rowBg = idx % 2 === 0 ? "bg-blue-950/55 hover:bg-blue-900/60" : "bg-blue-900/25 hover:bg-blue-900/40";

  const rankBlockClass = top3 ? top3.rank : "bg-cyan-950/60 text-cyan-300 border-cyan-500/30";
    ...
    <article key={rowKey} className={`rounded-md px-2.5 py-1.5 sm:px-3 sm:py-2 transition-colors ${rowBg}`}>
      <div className="flex items-center justify-between gap-2.5">
        ...
        <div className="shrink-0 text-right leading-tight">
          <p className="text-sm font-bold text-cyan-300">{value}</p>
        </div>
      </div>
    </article>
  );
