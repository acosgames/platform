...
import { addJoinQueues, clearGameQueues, getJoinQueues } from "./queue";
import { clearRooms, getLastJoinType, setLastJoinType } from "./room";
...
export async function createDisplayName({ displayname, portraitid, countrycode }: { displayname: string; portraitid: number; countrycode: string }) {
    try {
...
export async function createTempUser({ displayname, portraitid, countrycode }: { displayname: string; portraitid: number; countrycode: string }) {
    try {
...
export async function getPlayer(displayname: string) {
    try {
...
export async function loadUserGameData(game_slug: string) {
    try {
...
export function setLoginMode(user: { exp?: number; email?: string; displayname?: string } | null = null) {
...
function logoutTimer(user: PlayerPrivate) {
    let exp = user.exp;
